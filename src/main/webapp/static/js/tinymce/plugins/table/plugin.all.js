...
//调换ResizeBars和TableGrid生成顺序，使TableGrid中可以使用REsizeBars
 // Included from: js/tinymce/plugins/table/classes/ResizeBars.js
     ...
     define("tinymce/tableplugin/ResizeBars", [
         ...
         return function(editor) {
             ...
             function drawRows(rowPositions, tableWidth, tablePosition,contenteditable) {
                 Tools.each(rowPositions, function(rowPosition) {
                     var left = tablePosition.x,
                         ...
                         width = tableWidth;
                     if (editor.settings.disable_resize == false && contenteditable !="false") {
             ...
             function drawCols(cellPositions, tableHeight, tablePosition,contenteditable) {
             ...
             function drawBars(table) {
                 ...
                 drawRows(rowPositions, table.offsetWidth, tablePosition,table.contentEditable);
                 drawCols(colPositions, table.offsetHeight, tablePosition,table.contentEditable);
             }
             ...
             function adjustWidth(table, delta, index) {
                 ...
                 function setTableSize(newTableWidth, styleExtension, isPercentBased) {
                	 newTableWidth=newTableWidth+1;//每次获取的newTableWidth都比实际少1，不明原因，加上1，保证值不变
             ...
             return {
                 ...
                 getComputedStyleSize:getComputedStyleSize
             };
         };
     });
  ...
  // Included from: js/tinymce/plugins/table/classes/TableGrid.js
     ...
     define("tinymce/tableplugin/TableGrid", [
     ...
     ], function(Tools, Env, Utils, ResizeBars,SplitCols) {
         ...
         return function(editor, table, selectedCell) {
             ...
             function cloneCell(cell) {
                 ...
                 Tools.walk(cell, function(node) {
                     ...
                     if (node.nodeType == 3) {
                         ...
                         if (curNode) {
                             curNode.innerHTML = Env.ie && Env.ie < 11 ? '&nbsp;' : '<br data-mce-bogus="1" />';
                         }
                 ...
                 formatNode=null;//解决拆分后保存失败问题，拆分后的单元格不保留任何子元素（目前没发现需要保留的情况）
                 if (formatNode) {
             ...
             function split() {
                     ...
                     each(row, function(cell, x) {
                         ...
                         if (isCellSelected(cell)) {
                             ...
                             if (colSpan > 1 || rowSpan > 1) {
                                 ...
                                 //拆分时 将被拆分的单元格宽度平均分配到拆分后的单元格上
                                 var widthString = editor.dom.getStyle(cell, 'width');
                                 var width = parseInt(widthString, 10);
                                 editor.dom.setStyle(cell, 'width', (width/colSpan)+"px");
                                 // Insert cells right
               ...
               //add by yul 拆分后调用调整宽度函数，更新html中width值

                 resizeBars.adjustWidth(table,0,0);
             }
             ...
             function merge(cell, cols, rows) {
                 ...
                 if (startCell && endCell && startCell.part == endCell.part) {
					...
					//add by yul 合并后调用调整宽度函数，更新html中width值（只有adjustWidth函数中有相关更新内容，所以模拟调整表格）
                     resizeBars.adjustWidth(table,0,0);
                 }
             }
             ...
             function insertCol(before) {
                 ...
                 each(grid, function(row, y) {
                     ...
                     if (cell != lastCell) {
                         ...
                         if (colSpan == 1) {
                             if (!before) {
                                 var ncell = cloneCell(cell);
                                 dom.insertAfter(ncell, cell);
                                 fillLeftDown(posX, y, rowSpan - 1, colSpan);
                             } else {
                                 var ncell = cloneCell(cell);
                                 cell.parentNode.insertBefore(ncell, cell);
                                 fillLeftDown(posX, y, rowSpan - 1, colSpan);
                             }
                         } else {
                 ...
                 //add by yul style中的宽度变化后不自动更新，所以将第一排的宽度调整为实际宽度，后面的宽度调整为空
                 for(var j=0;j<table.rows.length;j++) {
                	 if(j==0){
                		 for(var i=0;i<table.rows[j].cells.length;i++) {
                        	 var w=resizeBars.getComputedStyleSize(table.rows[j].cells[i],'width');
                        	 editor.dom.setStyle(table.rows[j].cells[i],'width',w);
                         }
                     }else
                    	 {
                    	 for(var i=0;i<table.rows[j].cells.length;i++) {
                        	
                        	 editor.dom.setStyle(table.rows[j].cells[i],'width',0);
                         }
                    	 }
                	 }
                 resizeBars.adjustWidth(table,0,0);//使用调整宽度函数更新源代码
             }
             ...
             function deleteCols() {
                 ...
                 each(grid, function(row) {
                     each(row, function(cell, x) {
                         ...
                         editor.dom.setStyle(cell.elm,'width','');//删除后宽度不自动调整，先都清为空
                     });
                 });
            ...
            self.table = function(isProps) {
                ...
                function onSubmitTableForm() {
                    ...
                    editor.undoManager.transact(function() {
                        ...
                        //兼容IE10   border-color不可用   ---start
                        if(data.border!=""&&data.border!="0"){
                            var tdStyle = "1px solid "+data.borderColor;
                            var tableStyle = data.border+"px solid "+data.borderColor;
                        }else{
                            var tdStyle = "";
                            var tableStyle = "";
                        }
                        $(tableElm).css("border",tableStyle);
                        var trs =$(tableElm).find("tr");
                        for(var i=0, len=trs.length; i<len; i++){
                            if(data.border==""||data.border=="0"){
                                trs.eq(i).css("border-color","");
                            }
                        }
                        var tds =$(tableElm).find("td");
                        for(var i=0, len=tds.length; i<len; i++){
                            var nowBorderColor = tds.eq(i)[0].style.borderColor;
                            if(data.border==""||data.border=="0"||(nowBorderColor.indexOf("#")==-1&&nowBorderColor.indexOf("rgb")==-1)){
                                tds.eq(i).css("border",tdStyle);
                                var dataMceStyle = tds.eq(i).attr("data-mce-style") == undefined ? "" :tds.eq(i).attr("data-mce-style");
                                if (dataMceStyle.indexOf("border") != "-1") {
                                    dataMceStyle = dataMceStyle.replace(/border:.*;/, "border:"+tdStyle+";");
                                }
                                else {
                                    dataMceStyle += "border:"+tdStyle+";";
                                }
                                tds.eq(i).attr("data-mce-style", dataMceStyle);
                            }else{

                            }
                        }
                        //兼容IE10   border-color不可用   ---end
                ...
                function onSubmitCellForm() {
                    ...
                    editor.undoManager.transact(function() {
                        each(cells, function(cellElm) {
                            ...
                            //兼容IE10 border-color不可用---start
                            var tdStyle = "1px solid "+data.borderColor;
                            $(cellElm).css("border",tdStyle);
                            var dataMceStyle = $(cellElm).attr("data-mce-style") == undefined ? "" : $(cellElm).attr("data-mce-style");
                            if (dataMceStyle.indexOf("border") != "-1") {
                                dataMceStyle = dataMceStyle.replace(/border:.*;/, "border:"+tdStyle+";");
                            }
                            else {
                            ...
                            $(cellElm).attr("data-mce-style", dataMceStyle);
                            //兼容IE10 border-color不可用---end
                            // Switch cell type
        ...
        function Plugin(editor) {
            ...
            function insertTable(cols, rows) {
                ...
                for (y = 0; y < rows; y++) {
                    ...
                    for (x = 0; x < cols; x++) {
                        html += '<td>' + (Env.ie && Env.ie < 11 ? '&nbsp;' : '<br>') + '</td>';
                    }
                ...
                editor.undoManager.transact(function() {
					...
					//add by yul 合并后调用调整宽度函数，更新html中width值
                    editor.$('table', tableElm).each(function(index,table) {//由于tableElm为空，所以目前只能获取全部table，无法只选择新添加的table
                    	resizeBars.adjustWidth(table,0,0);
                    });
                   /* var table=editor.dom.doc.all[8];//tableElm值为空，需重新获取table值
                    var tab=editor.$('table', tableElm);
                    var table=editor.$('table', tableElm)[0];
                    resizeBars.adjustWidth(table,0,0);*/
                });
