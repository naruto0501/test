...
<form class="form">
    <div class="form-group">
        ...
        <input class="input-large" type="text" name="colLabel" id="colLabel">
    </div>

    <div class="form-group">
        <label class="control-label">字段英文名称：</label>
        <input type="text" name="colName" id="colName">
    </div>

    <div class="form-group">
        <label class="control-label">字段长度：</label>
        <input type="text" name="colLength" id="colLength">
    </div>
    <div class="form-group">
...
function initSelect(){
	...
	for (var o=0;o<ele.find("input").length;o++){
		if (ele.find("input").eq(o).attr("value")){
			...
			obj.lookupName = ele.find("input").eq(o).parent().text().replace(/(^\s*)|(\s*$)/g, "");
			array.push(obj);
		}
	}
	setSelect(array);
	...
	$("#relation_lookup").click(function(){
		 var dialog = new CommonJQueryUIDialog("syslookup","700","600", "avicit/platform6/eform/formdesign/js/plugins/radio-box/addLookup.jsp","选择通用代码", null, true, false, false);
		 dialog.show();
	});
